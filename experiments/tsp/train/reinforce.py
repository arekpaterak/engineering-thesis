...
class Args:
    ...
    expensive_action_threshold: float = 0.5
    """the threshold over which an action is ignored"""
        ...
        wandb.init(
            ...
            config={
                "env": {
                    ...
                    "max_t": args.max_t,
                    "expensive_action_threshold": args.expensive_action_threshold,
                },
                "algorithm": {
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if args.track:
            wandb.log(
            {
                    ...
                    "ignored_actions": n_ignored_actions,
                    "avg_action_expense": np.mean([sum(action) for action in actions])
                },
                step=epoch
            )

        if args.debug:
            print(f"Epoch {epoch:4} Loss: {loss} Average Score: {np.mean(scores_deque):.2f}")
            print(f"n_ignored_actions: {n_ignored_actions}")
            print(f"best_objective_value: {info['best_objective_value']}")
            print(f"avg_action_expense: {np.mean([sum(action) for action in actions])}")
